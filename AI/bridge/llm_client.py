...
2.출력 형식의 element의 채울만한 내용이 없다면 null값을 채우세요. 
[반드시 지킬 것]
...
async def veoprompt_generate(payload: Dict[str, Any]) -> str:
    ...
    be = Di.get("beforeprompt")
    wt = Di.get("weather")
    sa = Di.get("sample")
    user_note = Di.get("user")
    ...
    for candidate in (Di, Di.get("options"), Di.get("element"), sa):
        if isinstance(candidate, dict):
            data_sources.append(candidate)

    def _first_value(keys: list[str]) -> Optional[str]:
    ...
    camera_motion = _first_value(["camera_motion", "cameraMotion"]) or "None"
    subject_animation = _first_value(["subject_animation", "subjectAnimation"]) or "None"
    environmental_animation = _first_value(["environmental_animation", "environmentalAnimation"]) or "None"
    sound_effects = _first_value(["sound_effects", "soundEffects"]) or "None"
    dialogue = _first_value(["dialogue"]) or ""

    keywords: list[str] = []
    for opt in (camera_motion, subject_animation, environmental_animation, sound_effects):
    ...
    if be:
        context_sections.append(f"Existing guidance: {_clean(be)}")
    if extract:
        context_sections.append("Extracted elements: " + _clean(extract))
    if wt:
        context_sections.append("Weather context: " + _clean(wt))
    if sa:
        context_sections.append("Sample reference: " + _clean(sa))
    if user_note:
        context_sections.append("User note: " + _clean(user_note))

    context_text = "\n".join(cs for cs in context_sections if cs)
    mandatory_kw = ",".join(dict.fromkeys(keywords)) if keywords else "None"

    gemini_prompt = (
        "You are an expert prompt engineer for Google's Veo model. "
        "Analyze the provided image and combine its content with the following motion and audio keywords to generate a single, cohesive, and cinematic prompt. "
        ...
        f"Mandatory Keywords: {mandatory_kw}"
    )
    if context_text:
        gemini_prompt = f"{gemini_prompt}\n{context_text}"

    def _resolve_image_bytes(img_ref: Any) -> tuple[Optional[bytes], Optional[str]]:
        if not img_ref:
            return None, None
        if isinstance(img_ref, (bytes, bytearray)):
            return bytes(img_ref), "image/png"
    ...
    image_bytes, image_mime = _resolve_image_bytes(Di.get("img"))

    contents: list[Any] = [gemini_prompt]
    if image_bytes:
        contents.append(types.Part.from_bytes(data=image_bytes, mime_type=image_mime or "image/png"))
    ...
    for attempt in range(3):
        try:
            ...
            if attempt < 2:
                ...
                continue
            raise RuntimeError(f"Gemini client failed: {e}") from e
